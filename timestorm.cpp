...
int main(){
    ...
    while(dis > 0 && time_loop==true && ship==true){
        ...
        if (command_center==1){
            ...
            dis-=2;
            int nausea = nauseous_prompt(dis, looped, "You start to feel a little bit more nauseous, probably because you haven't really recovered much from the torture you endured.", "Head back to bunk bed to rest.", "Go to the lab to get checked out", "Continue to search the whole place for clues as to was is currently happening", "Realize that you are time looping.");
            looped = true;
            if (nausea==1){
                dis-=2;
